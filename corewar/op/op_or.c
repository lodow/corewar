...
int	op_or_dir(t_process *proc, t_vm *vm, int param)
{
  int	dir;
  int	at;
...
int	op_or_ind(t_process *proc, t_vm *vm, int param)
{
  short	adrr;
  int	dir;
  int	at;
...
int	op_or_reg(t_process *proc, t_vm *vm, int param)
{
  int	reg;
      ...
      while ((i < 2) && (GET_TYPE_PARAMX(PARAMBYTE, i) != 00))
        {
          if (GET_TYPE_PARAMX(PARAMBYTE, i) == 1)
            val[i] = op_or_reg(proc, vm, i);
          else if(GET_TYPE_PARAMX(PARAMBYTE, i) == 2)
            val[i] = op_or_dir(proc, vm, i);
          else
            val[i] = op_or_ind(proc, vm, i);
          i++;
        }
